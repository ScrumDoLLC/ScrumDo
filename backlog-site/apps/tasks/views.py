...
def tasks(request, group_slug=None, template_name="tasks/task_list.html", bridge=None):
    ...
    try:
        project = bridge.get_group(group_slug)
    ...
    backlog_items = project.tasks
    iteration_items = project.tasks


    
    return render_to_response(template_name, {
       'backlog':backlog_items,
       'iteration':iteration_items
    ...
    if request.method == "POST":
        if request.user.is_authenticated():
            ...
            if task_form.is_valid():
                ...
                task.group = group
                task.iteration = group.get_default_iteration()
                task.save()
                task.save_history()
