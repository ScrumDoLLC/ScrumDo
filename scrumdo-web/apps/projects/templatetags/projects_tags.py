...
def tiny_google_chart_url(iteration_or_project):
    return _google_chart(iteration_or_project, project_size="200x30", iteration_size="200x30", label_axis="", bg_color="f6f6f6", axis_color="f6f6f6")

def _google_chart(iteration_or_project, project_size="550x120", iteration_size="550x80", label_axis="y", bg_color="ffffff", axis_color="444444"):
    ...
    try:
        ...
        data = "http://chart.googleapis.com/chart?chf=bg,s,%s&chxr=0,0,%d&cht=lxy&chs=%s&chd=s:%s,%s,%s,%s&chxt=%s,x&chxs=0,%s,8,0,lt&chxl=1:|%s|%s&chco=9ED147,197AFF&chm=B,eef5fb,1,0,0|B,99CBB0,0,0,0" % ( bg_color, max_val,size,"".join(claimed_dates), "".join(claimed_values), "".join(total_dates), "".join(total_values), label_axis, start_date_s, end_date_s, axis_color )
        #logger.debug(data)
        return data
    except:
        return "http://chart.googleapis.com/chart?cht=lxy&chs=1x1"
